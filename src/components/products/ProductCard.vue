...
<template>
  <div class="flex flex-col w-[200px] min-w-[200px] h-[330px] overflow-hidden product-card-shadow bg-white rounded-xl">
    <img class="h-[180px] object-cover w-full" :src="product.image" alt="" />
    <div class="flex-auto p-3">
      <p class="text-xs font-semibold text-[#363636] truncate-2">{{ product.name }}</p>
      ...
      <span class="text-[#b0b0b0] text-xs px-1 py-1">$100.00</span>
      <div>
        <i class="ri-map-pin-2-fill text-[#b0b0b0]"></i>
        <span class="ml-2 text-xs text-[#b0b0b0]">{{ product.location }}</span>
      </div>
      <div class="text-[#b0b0b0] text-xs">
        <span>
          <i class="ri-star-fill text-[#ffaa28]"></i>
          <span class="ml-2 mr-1">{{ product.star }}</span>
          <span>|</span>
